...
def pca_temp(df_merged, m):
    ...
    Return : 
     - df_pca -> data frame, jeu de donnée projeté en 2 dimensions
    """
...
def mds_temp(df_merged, m, n):
    ...
    Return : 
     - df_mds -> data frame, jeu de donnée projeté en 1 dimension puis exprimé avec le time stamp
    """
    T = df_merged(['time'])
    df_merged.drop(['time'], axis=1, inplace=True)
...
def mesure_larges(df_syn, df_rel):
    """
    retourne dictionnaire des distances + kullback leibler
    """
